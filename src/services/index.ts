 ...
 *
 * Every page/hook consumes `services.*`. Each method tries the real Node/Express
 * endpoint first (when VITE_API_BASE_URL is set) and otherwise resolves the
 * seeded demo dataset. Swapping demo → Node/Express requires no page changes.
 */
 ...
 *  - "live"      : VITE_API_BASE_URL is set and the Node/Express backend answers.
 ...
 * browser. A real deployment authenticates against the Node/Express /auth/login
...
const auth = {
  async login({ email, password }: LoginPayload): Promise<{ user: AuthUser; token: string }> {
    if (IS_LIVE_BACKEND) {
      try {
        ...
        const backendRole = String(res.user.role).toLowerCase();
        const normalizedRole: Role =
          backendRole === "admin" || backendRole === "administrator"
            ? "Administrator"
            : backendRole === "user"
              ? "SOC Analyst L1"
              : (res.user.role as Role);

        const normalizedStatus =
          res.user.status ??
          (res.user.approved === true ? "active" : "pending");
        ...
        return {
          token: res.token,
          user: { ...user, permissions: permissionsFor(normalizedRole) },
        };
      } catch (error) {
        // Authentication must never silently fall back to demo credentials
        // when a live backend is configured. A live backend error is real.
        throw error;
      }
    }
  ...
  async register(input: { name: string; email: string; password: string }): Promise<User> {
    if (IS_LIVE_BACKEND) {
      ...
      return {
        ...res.user,
        status: res.user.approved === true ? "active" : "pending",
        lastLogin: null,
      } as User;
    }
    const created: User = {
...
const reports = {
  ...
  /**
   * Downloads a report. Tries GET /reports/{id}/export?format=… on the Node/Express
...
const admin = {
  ...
  async users(): Promise<User[]> {
    if (!IS_LIVE_BACKEND) return demo(getDemoDataset().users);

    const rows = await apiFetch<Array<User & { approved?: boolean }>>("/admin/users");
    setDataMode("live");

    return rows.map((u) => ({
      ...u,
      role:
        ["admin", "administrator"].includes(String(u.role).toLowerCase())
          ? "Administrator"
          : String(u.role).toLowerCase() === "user"
            ? "SOC Analyst L1"
            : (u.role as Role),
      status: u.approved === true ? "active" : "pending",
  ...
  async updateUser(id: string, patch: Partial<User>) {
    ...
    // The current production backend exposes explicit approval rather than
    // accepting arbitrary user-role/status patches.
    ...
    throw new ApiError(
      400,
      "This production backend currently supports account approval only.",
    );
  },
